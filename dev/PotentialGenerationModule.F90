...
!
   ...
   real (kind=RealKind), allocatable ::  kri(:)
   real (kind=RealKind), allocatable, target ::  Bj_l(:)
   type (ChebyshevStruct), allocatable, target :: chebv_struct(:)
!
   interface
      subroutine constructDataOnGrid(grid_type, value, value_type, getData, skey, lmax, spin)
...
!
   if ( isFullPot ) then
      allocate( dr_int(nr_int_max), d_ir(nr_int_max) )
      allocate( Ylm(kmax_max) )
      allocate( Bj_l(n_interp_max*nr_int_max*(lmax_max+1)) )
      allocate( kri(n_interp_max*nr_int_max) )
      allocate( ind_interp(n_interp_max*nr_int_max) )
      allocate( r_interp(n_interp_max*nr_int_max) )
      allocate( v_interp(n_interp_max*nr_int_max*jmax_max) )
...
!
   ...
   if ( isFullPot ) then
      ...
      deallocate( ind_interp, r_interp, v_interp )
...
!  ===================================================================
   ...
   use ChargeDensityModule, only : getChargeDensity, getMomentDensity, &
                                   ...
                                   getRhoLmax
   use ExchCorrFunctionalModule, only : getExchCorrPot
   use Uniform3DGridModule, only : getNumGridPoints, getPointDomainIndicator
...
!
   nga = getNumGridPoints('FFT',1)
   ngb = getNumGridPoints('FFT',2)
   ngc = getNumGridPoints('FFT',3)
   ng  = getNumGridPoints('FFT')
!
   call constructDataOnGrid( 'FFT', 'Charge', 'TotalNew', getChargeDensityAtPoint, StorageKey )
!
   p_rho => getDataStorage(StorageKey, nga, ngb, ngc, RealMark)
!
   if ( n_spin_pola==2 ) then
      call constructDataOnGrid( 'FFT', 'Moment', 'TotalNew', getMomentDensityAtPoint, StorageKey )
!
      p_mom => getDataStorage(StorageKey, nga, ngb, ngc, RealMark)
!
   endif
!
      ...
      do nc = 1,ngc
         do nb = 1,ngb
            do na = 1,nga
               ...
               if ( getPointDomainIndicator('FFT',nn,1)==0 ) then
                  nn_av = nn_av + 1
...
!
   ...
   do is = 1,n_spin_pola
      call constructDataOnGrid( 'FFT', 'Potential', 'XCHat', getPotentialAtPoint, StorageKey, lmax=0, spin=is )
!
      p_Vexch => getDataStorage(StorageKey, nga, ngb, ngc, RealMark)
!
      if ( is == 2 ) then
         p_rho => p_mom
      endif
...
!           ---------------------------------------------------------
            call calRadialProjection( lmax, jend, r_mesh, rmt, posi, &
...
!  ===================================================================
   ...
   use ParallelFFTModule, only : performTransformR2C
   use AtomModule, only : getLocalAtomPosition
...
!
   call constructDataOnGrid( 'FFT', 'Charge', 'Pseudo', getChargeDensityAtPoint, StorageKey )
#ifdef TIMING
   t1 = getTime()
...
#endif
   dfp_total = ZERO
This needs to be revisited to make sure that the loop is over all atoms!!!
   do id = 1,LocalNumAtoms
      posi   = getLocalAtomPosition(id)
!     ----------------------------------------------------------------
      call calPseudoDipoleField( posi, fft_c, dfp)
!     ----------------------------------------------------------------
...
!
   do id = 1,LocalNumAtoms
      ...
      v_jl = CZERO
!     ----------------------------------------------------------------
      call calRadialProjection( lmax, nr, r_mesh, rmt, posi, FFT_data,   &
                                -2, 60, v_jl)
!     ----------------------------------------------------------------
!
      do jl = 1,jmax
...
!
!  ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
   subroutine calPseudoDipoleField(posi, fft_c, dfp)
!  ===================================================================
...
!
   complex (kind=CmplxKind), intent(in) :: fft_c(:)
   complex (kind=CmplxKind) :: expikR, cfact
!
   integer (kind=IntKind) :: idk, idk0, nump_local, comm
!
   nump_local = getNumLocalGridPoints('K')
   ...
   do idk =1, idk0-1
      ...
      expikR  = exp( -SQRTm1*(kvec(1)*posi(1) + kvec(2)*posi(2) + p_kvec(3)*posi(3)) )
      dfp = dfp + real(cfact*expikR*fft_c(idk),kind=RealKind)*kvec/kv2
   enddo
   do idk =k0+1, nump_local
      kvec = getGridPointCoord('K',idk)
      kv2 = kvec(1)**2+kvec(2)**2+kvec(3)**2
      expikR  = exp( -SQRTm1*(kvec(1)*posi(1) + kvec(2)*posi(2) + p_kvec(3)*posi(3)) )
      dfp = dfp + real(cfact*expikR*fft_c(idk),kind=RealKind)*kvec/kv2
...
!
!  -------------------------------------------------------------------
   comm = getParaFFTCommunicator()
   call GlobalSumInGroup(comm,dfp,3)
!  -------------------------------------------------------------------
!
...
!
   ...
   do is = 1,n_spin_pola
      call constructDataOnGrid( 'FFT', 'Potential', 'En_XCHat', getPotentialAtPoint, StorageKey, lmax=0, spin=is )
!
      p_Eexch => getDataStorage(StorageKey, nga, ngb, ngc, RealMark)
!
      if ( is == 2 ) then
         p_rho => p_mom
      endif
...
!
!  ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
   subroutine calRadialProjection( lmax, n_rmesh, rmesh, rmt_in,      &
                                   posi_in, fft_c, kpow, ninterp, pv_jl )
!  ===================================================================
...
!
   integer (kind=IntKind) :: idk, idk0, nump_local, ng, nnr
   integer (kind=IntKind) :: i, j, k, ind_n, nr
...
!
   ...
   real (kind=RealKind), intent(in), optional :: rmt_in
!
   real (kind=RealKind) :: dummy, k2, k2pow, posi(3), rmt, kvec(3), kfact_r, kfact_i
...
!
   nnr = n_interp*nr_int
!
   p_Bj_l(:,0:) => aliasArray2_c( Bj_l, nnr, lmax+1) )
!
   do i = 1,nr_int
      r0 = dr_int(i)
!     ----------------------------------------------------------------
...
!
   n_sz = nnr*jmax
   pv_interp  => aliasArray2_c( v_interp, nnr, jmax )
!
   posi = posi_in
!
...
!
   if (kpow < 0) then
      idk0 = getLocalIndexOfGridOrigin('K')
      if ( idk0 /= 1) then
         call ErrorHandler('calRadialProjection','Grid origin does not correspond to the first local index',idk0)
      endif
      idk0 = 2
   else
      idk0 = 1
...
!
      if (kpow == -2) then
         ...
         kfact = (expikR*theta_k)*sqrt(k2)**kpow
      endif
...
!     ----------------------------------------------------------------
      ...
      call SphericalBessel( lmax, nnr, kri, p_Bj_l )
!     ----------------------------------------------------------------
!
      do l = 0,lmax
         if (mod(l,2) == 0) then
            ...
            do m = 0,l
               kl = (l+1)*(l+1) -l + m
               jl = ((l+1)*(l+2))/2 -l + m
               pv_interp(1:nnr,jl) = pv_interp(1:nnr,jl) + &
                                                 sqrtm1*kfact_i*Ylm(kl)*p_Bj_l(1:nnr,l)
             enddo
         endif
      enddo
   enddo
!  -------------------------------------------------------------------
   call GlobalSumInGroup(comm,pv_interp,nnr,jmax)
!  -------------------------------------------------------------------
!
!  -------------------------------------------------------------------
   ng = getNumGlobalGridPoints()
!  -------------------------------------------------------------------
   if ( kpow == -2) then
...
!     ----------------------------------------------------------------
      call ErrorHandler('calRadialProjection','Unconventional situation', &
...
!
   nullify( pv_interp, pv, pv0, p_Bj_l )
!
